...
If it is already a filter, it will be removed\n""")
    ...
    if type(growthActive) != str: #For growth rate range
        growthRange = ((growthActive[0] < data[:,1]) & (data[:,1] < growthActive[1]))
        data = data[growthRange] #Data is filtered for growth rate range

    if type(tempActive) != str: #For temperature range
        self.tempRange = ((tempActive[0] < data[:,0]) & (data[:,0] < tempActive[1]))
        data = data[self.tempRange] #Data is filtered for temperature range
